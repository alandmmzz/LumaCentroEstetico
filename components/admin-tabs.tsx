...
const tabs = ["turnos", "servicios", "personal"] as const

export function AdminTabs({ turnos, servicios, personal }: { turnos: ReactNode; servicios: ReactNode; personal: ReactNode }) {
  const [active, setActive] = useState<(typeof tabs)[number]>("turnos")
  const content = { turnos, servicios, personal }[active]
  return <div>
