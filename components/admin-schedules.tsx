...
export function AdminSchedules({ schedules, catalog }: { schedules: Array<{ serviceCategory: string; startTime: string }>; catalog: Catalog }) {
  ...
  return <section aria-labelledby="gestionar-horarios"><div className="mb-8 flex items-end justify-between gap-4"><div><p className="text-xs uppercase tracking-[0.3em] text-primary">Agenda configurable</p><h2 id="gestionar-horarios" className="mt-2 font-serif text-4xl text-foreground">Horarios por servicio</h2></div>{message && <p role="status" className="text-sm text-primary">{message}</p>}</div><div className="grid gap-5 md:grid-cols-3">{ONLINE_CATEGORIES.map((category) => <article key={category} className="rounded-xl border border-border bg-card p-5"><h3 className="font-serif text-2xl text-foreground">{category}</h3><div className="mt-3 border-b border-border/60 pb-4"><p className="text-xs text-muted-foreground">Todos los tratamientos de este servicio duran lo mismo.</p><p className="mt-2 text-sm text-primary">Duración de agenda: {catalog.find((item) => item.name === category)?.durationMinutes ?? 90} min</p></div><div className="mt-5 flex flex-col gap-3">{(values[category] ?? []).map((time, index) => <div key={`${category}-${index}`} className="flex items-center gap-2"><input type="time" value={time} onChange={(event) => setValues((current) => ({ ...current, [category]: current[category].map((item, itemIndex) => itemIndex === index ? event.target.value : item) }))} className="flex-1 rounded-md border border-input bg-background px-3 py-2 text-sm" /><button type="button" onClick={() => setValues((current) => ({ ...current, [category]: current[category].filter((_, itemIndex) => itemIndex !== index) }))} className="text-sm text-muted-foreground hover:text-destructive" aria-label={`Eliminar horario ${time}`}>Eliminar</button></div>)}</div><div className="mt-5 flex gap-2"><button type="button" onClick={() => add(category)} className="rounded-full border border-primary/40 px-4 py-2 text-sm text-primary">Agregar horario</button><button type="button" onClick={() => save(category)} disabled={pending} className="rounded-full bg-primary px-4 py-2 text-sm text-primary-foreground">Guardar</button></div></article>)}</div></section>
}
