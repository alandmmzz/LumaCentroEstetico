          ...
          {bookingCategories.map((item) => {
            ...
            return (
              <div key={item.name}>
                ...
                <div className={`grid transition-[grid-template-rows,opacity] duration-300 ease-out ${active ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"}`}><div className="min-h-0 overflow-hidden"><div className="mt-2 rounded-lg border border-primary/25 bg-background/70 p-3" role="group" aria-label={`Tratamientos de ${item.name}`}><div className="mb-2 flex items-center gap-2 text-xs uppercase tracking-wider text-primary"><ChevronDown className={`h-3.5 w-3.5 transition-transform duration-300 ${active ? "rotate-0" : "-rotate-90"}`} /> Elegí uno o más tratamientos</div><div className="flex flex-col gap-1">{item.treatments.map((treatment) => { const checked = selectedTreatments.includes(treatment.id); const treatmentNote = (("note" in treatment ? treatment.note : "") || treatmentNotes[treatment.name] || "Información sobre este tratamiento disponible en LUMA."); return <label key={treatment.id} className="flex cursor-pointer items-start gap-3 rounded-md px-2 py-2 hover:bg-primary/5"><input type="checkbox" checked={checked} onChange={() => toggleTreatment(treatment.id)} className="sr-only" /><span className={`mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded border ${checked ? "border-primary bg-primary text-primary-foreground" : "border-border"}`}>{checked && <Check className="h-3 w-3" aria-hidden="true" />}</span><span className="group relative min-w-0 flex-1 text-sm text-foreground"><span className="inline"><span>{treatment.name}</span><button type="button" aria-label={`Información sobre ${treatment.name}`} aria-expanded={openInfo === String(treatment.id)} onClick={(event) => { event.preventDefault(); event.stopPropagation(); setOpenInfo(openInfo === String(treatment.id) ? null : String(treatment.id)) }} className="peer inline-flex size-5 items-center justify-center rounded-full border border-primary/60 text-xs font-medium text-primary transition-colors hover:bg-primary hover:text-primary-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">i</button></span>{treatmentNote && <span role="tooltip" className={`mt-1 block w-full rounded-md border border-border bg-background p-3 text-xs leading-relaxed text-muted-foreground shadow-sm transition-all md:pointer-events-none md:mt-1 md:max-h-0 md:overflow-hidden md:p-0 md:opacity-0 md:shadow-none md:group-hover:max-h-24 md:group-hover:p-3 md:group-hover:opacity-100 md:delay-500 ${openInfo === String(treatment.id) ? "max-h-24 opacity-100" : "max-h-0 overflow-hidden opacity-0 p-0"}`}>{treatmentNote}</span>}</span><span className="text-sm tabular-nums text-primary">{treatment.price === null ? "Consultar" : ("promoPrice" in treatment && treatment.promoPrice != null) ? <><span className="mr-1 text-muted-foreground line-through">{formatUYU(treatment.price)}</span>{formatUYU(Number(treatment.promoPrice))}</> : formatUYU(treatment.price)}</span></label> })}</div></div></div></div>
              </div>
            )
          })}
        </div>
