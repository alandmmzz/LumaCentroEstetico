...
const treatmentNotes: Record<string, string> = {
  "Esmaltado semipermanente": "Esmaltado de larga duración para mantener tus uñas prolijas y brillantes.",
  "Kapping gel": "Refuerzo con gel que aporta resistencia y una terminación natural.",
  "Mantenimiento kapping": "Mantenimiento del refuerzo para conservar tus uñas fuertes y prolijas.",
  "Nivelación rubber": "Base flexible que nivela la uña y ayuda a protegerla.",
  Softgel: "Extensiones livianas y naturales para lucir uñas largas y cuidadas.",
  "Mantenimiento softgel": "Retoque de tus extensiones softgel para mantener su forma y duración.",
  "Remoción de retiros ajenos": "Retiro cuidadoso de productos realizados en otro salón.",
  "Remoción de LUMA": "Retiro cuidadoso del producto aplicado en LUMA.",
  "Tratamiento Peeling": "Exfoliación química suave que ayuda a renovar la piel, mejorar su textura y devolver luminosidad.",
  "Limpieza + Dermaplaning": "Limpieza profunda combinada con dermaplaning para retirar impurezas y células superficiales.",
  "Limpieza profunda": "Higiene facial profunda para desobstruir poros, equilibrar la piel y mejorar su aspecto.",
  "Spa facial": "Ritual hidratante y revitalizante para devolverle frescura y luminosidad a tu piel.",
  "Hidratación facial": "Tratamiento nutritivo que ayuda a recuperar la suavidad y elasticidad de la piel.",
  "Radiofrecuencia facial": "Tratamiento con radiofrecuencia que estimula la piel y mejora su firmeza.",
}
          ...
          {bookingCategories.map((item) => {
            ...
            return (
              <div key={item.name}>
                ...
                <div className={`grid transition-[grid-template-rows,opacity] duration-300 ease-out ${active ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"}`}><div className="min-h-0 overflow-hidden"><div className="mt-2 rounded-lg border border-primary/25 bg-background/70 p-3" role="group" aria-label={`Tratamientos de ${item.name}`}><div className="mb-2 flex items-center gap-2 text-xs uppercase tracking-wider text-primary"><ChevronDown className={`h-3.5 w-3.5 transition-transform duration-300 ${active ? "rotate-0" : "-rotate-90"}`} /> Elegí uno o más tratamientos</div><div className="flex flex-col gap-0.5">{item.treatments.map((treatment) => { const checked = selectedTreatments.includes(treatment.id); const treatmentNote = (("note" in treatment ? treatment.note : "") || treatmentNotes[treatment.name] || "Información sobre este tratamiento disponible en LUMA."); return <label key={treatment.id} className="grid grid-cols-[1rem_minmax(0,1fr)_auto] items-start gap-x-2 gap-y-0.5 rounded-md px-2 py-1 hover:bg-primary/5"><input type="checkbox" checked={checked} onChange={() => toggleTreatment(treatment.id)} className="sr-only" /><span className={`mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded border ${checked ? "border-primary bg-primary text-primary-foreground" : "border-border"}`}>{checked && <Check className="h-3 w-3" aria-hidden="true" />}</span><span className="group relative min-w-0 flex-1 text-sm text-foreground"><span className="inline"><span>{treatment.name}</span><button type="button" aria-label={`Información sobre ${treatment.name}`} aria-expanded={openInfo === String(treatment.id)} onClick={(event) => { event.preventDefault(); event.stopPropagation(); setOpenInfo(openInfo === String(treatment.id) ? null : String(treatment.id)) }} className="peer ml-2 inline-flex size-4 shrink-0 -translate-y-px items-center justify-center rounded-full border border-primary/60 text-[10px] font-medium text-primary transition-colors hover:bg-primary hover:text-primary-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">i</button></span>{treatmentNote && <span role="tooltip" className={`col-span-3 mt-1 block w-full rounded-md border border-border bg-background p-3 text-xs leading-relaxed text-muted-foreground shadow-sm transition-all md:pointer-events-none md:col-span-1 md:mt-1 md:max-h-0 md:overflow-hidden md:p-0 md:opacity-0 md:shadow-none md:group-hover:max-h-24 md:group-hover:p-3 md:group-hover:opacity-100 md:delay-500 ${openInfo === String(treatment.id) ? "max-h-24 opacity-100" : "max-h-0 overflow-hidden opacity-0 p-0"}`}>{treatmentNote}</span>}</span><span className="text-sm tabular-nums text-primary">{treatment.price === null ? "Consultar" : ("promoPrice" in treatment && treatment.promoPrice != null) ? <><span className="mr-1 text-muted-foreground line-through">{formatUYU(treatment.price)}</span>{formatUYU(Number(treatment.promoPrice))}</> : formatUYU(treatment.price)}</span></label> })}</div></div></div></div>
              </div>
            )
          })}
        </div>
