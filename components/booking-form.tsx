...
export function BookingForm({ catalog }: { catalog?: BookingCatalog }) {
  ...
  useEffect(() => {
    ...
    Promise.all([getBookedTimes(selectedDate, selectedCategory), getAvailableSchedule(selectedCategory)]).then(([available, schedule]) => { setAvailableSchedule(available); setBookedTimes(schedule.filter((time) => !available.includes(time))) }).finally(() => setIsLoadingTimes(false))
