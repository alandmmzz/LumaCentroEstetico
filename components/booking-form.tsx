          ...
          {bookingCategories.map((item) => {
            ...
            return (
              <div key={item.name}>
                ...
                <div className={`grid transition-[grid-template-rows,opacity] duration-300 ease-out ${active ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"}`}><div className="min-h-0 overflow-hidden"><div className="mt-1 rounded-lg border border-primary/25 bg-background/70 p-2" role="group" aria-label={`Tratamientos de ${item.name}`}><div className="mb-1 flex items-center gap-2 text-xs uppercase tracking-wider text-primary"><ChevronDown className={`h-3.5 w-3.5 transition-transform duration-300 ${active ? "rotate-0" : "-rotate-90"}`} /> Elegí uno o más tratamientos</div><div className="flex flex-col gap-1">{item.treatments.map((treatment) => { const checked = selectedTreatments.includes(treatment.id); const treatmentNote = ("note" in treatment ? treatment.note?.trim() : "") ?? ""; return <label key={treatment.id} className="grid grid-cols-[1rem_minmax(0,1fr)_auto] items-start gap-x-2 gap-y-0 rounded-md px-1 py-0.5 hover:bg-primary/5"><input type="checkbox" checked={checked} onChange={() => toggleTreatment(treatment.id)} className="sr-only" /><span className={`mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded border ${checked ? "border-primary bg-primary text-primary-foreground" : "border-border"}`}>{checked && <Check className="h-3 w-3" aria-hidden="true" />}</span><span className="group relative min-w-0 flex-1 text-sm text-foreground"><span className="inline"><span>{treatment.name}</span>{treatmentNote && <Info className="ml-1 inline-block size-2.5 translate-y-px text-primary/75" strokeWidth={2.25} aria-hidden="true" />}</span>{checked && treatmentNote && <span className="col-span-3 mt-0.5 block w-full text-xs leading-relaxed text-muted-foreground">{treatmentNote}</span>}</span><span className="text-sm tabular-nums text-primary">{treatment.price === null ? "Consultar" : ("promoPrice" in treatment && treatment.promoPrice != null) ? <><span className="mr-1 text-muted-foreground line-through">{formatUYU(treatment.price)}</span>{formatUYU(Number(treatment.promoPrice))}</> : formatUYU(treatment.price)}</span></label> })}</div></div></div></div>
              </div>
            )
          })}
        </div>
