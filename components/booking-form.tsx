      ...
      <fieldset>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {SERVICE_CATEGORIES.map((item) => {
            ...
            return (
              <div key={item.name}>
                <button type="button" role="radio" aria-checked={active} onClick={() => chooseCategory(item.name)} className={`flex w-full items-center gap-3 rounded-lg border p-4 text-left transition-all ${item.name === "Promos" ? "border-primary/70 bg-primary/10 shadow-sm hover:border-primary" : active ? "border-primary bg-primary/10" : "border-border bg-card hover:border-primary/50"}`}>
                  <Icon className="h-5 w-5 shrink-0 text-primary" aria-hidden="true" />
                  <span className="flex-1"><span className="flex flex-wrap items-center gap-2 font-serif text-lg text-foreground">{item.name}{item.name === "Promos" && <span className="rounded-full bg-primary px-2 py-0.5 font-sans text-[10px] uppercase tracking-wider text-primary-foreground">COMBINACIONES ESPECIALES</span>}</span><span className="block text-xs text-muted-foreground">{item.description}</span></span>
